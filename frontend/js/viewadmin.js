...
function renderDoctors(doctors) {
    ...
    doctors.forEach(doctor => {
        ...
        removeButton.addEventListener('click', () => {
            ...
            fetch(`${backendURL}doctors/delete/${doctor._id}`, requestOptions)
                .then(response => {
                    if (response.ok) {
                        alert("Doctor removed")
                        doctorCard.remove(); // Use doctorCard to remove doctor from HTML
                    } else {
...
function renderPatients(patients) {
    ...
    fetch(`${backendURL}patients/delete/${patient._id}`, requestOptions)
        .then(response => {
            if (response.ok) {
                alert("Patient removed");
                patientCard.remove(); // Now it correctly removes the patient card from the DOM
            } else {
